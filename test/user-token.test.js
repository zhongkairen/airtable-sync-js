...
describe('UserToken', () => {
    describe('constructor', () => {
        it('should initialize with token from environment variable', () => {
            ...
            const userToken = new UserToken({ token: 'TEST_TOKEN' }, {});
            expect(userToken.token).to.equal('test-token-value');
            delete process.env.TEST_TOKEN;
        });

        it('should initialize with token path from environment variable', () => {
            process.env.TEST_TOKEN_PATH = '/test/path';
            const userToken = new UserToken({ tokenPath: 'TEST_TOKEN_PATH' }, {});
        ...
        it('should throw an error if neither token nor token path is set', () => {
            expect(() => new UserToken({ token: 'NON_EXISTENT_TOKEN', tokenPath: 'NON_EXISTENT_PATH' }, {})).to.throw(Error);
        });
    });
});